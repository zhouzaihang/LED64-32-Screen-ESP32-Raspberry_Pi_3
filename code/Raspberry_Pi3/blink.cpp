...
void hc595senddata(byte datar21, byte datar22, byte datar11, byte datar12)
{
  for (byte i = 0; i < 8; i++)
  {
	//The first screen
...
void display()
{
  for (Rownumber = 0; Rownumber < 16; Rownumber = Rownumber + 1)
  {
    for (int i = 0; i < 8; i++)
    {
      hc595senddata(ShowR[Rownumber*8+i], ShowR[Rownumber*8+i+128],ShowR[Rownumber*8+i+256], ShowR[Rownumber*8+i+384]);
    }
...
int main (void)
{
	...
	while(1)
	{
		...
		if (msgrcv(msgqid, &msg,SIZE+32,0,IPC_NOWAIT) != -1)
		{
			memcpy(&ShowR[0], &msg.msg_text[0],4096);
		}
